...
const { execute } = require('../../../src/cmd_processor/commands/stop') as {
...
describe('stop command', () => {
  ...
  it('publishes stop event when user is in a voice channel', async () => {
    const interaction = createMockInteraction({ channelId: '12345' });
    await execute(interaction);

    expect(interaction.reply).toHaveBeenCalledWith('Clearing the queue');
...
it('loads module without DEBUG set (covers false branch of DEBUG ternary)', () => {
  ...
  jest.isolateModules(() => {
    ...
    require('../../../src/cmd_processor/commands/stop');
  });
