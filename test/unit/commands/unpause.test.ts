...
const { execute } = require('../../../src/cmd_processor/commands/unpause') as {
...
describe('unpause command', () => {
  ...
  it('publishes unpause event when user is in a voice channel', async () => {
    const interaction = createMockInteraction({ channelId: '12345' });
    await execute(interaction);

    expect(interaction.reply).toHaveBeenCalledWith('Unpausing queue');
    expect(redisClient.publish).toHaveBeenCalledWith(
      CHANNEL_EVENT_KEY,
      expect.stringContaining('"type":"unpause"')
    );
...
it('loads module without DEBUG set (covers false branch of DEBUG ternary)', () => {
  ...
  jest.isolateModules(() => {
    // eslint-disable-next-line @typescript-eslint/no-require-imports
    require(`../../../src/cmd_processor/commands/unpause`);
  });
