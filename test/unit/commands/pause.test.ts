...
const { execute } = require('../../../src/cmd_processor/commands/pause') as {
...
describe('pause command', () => {
  ...
  it('publishes pause event when user is in a voice channel', async () => {
    const interaction = createMockInteraction({ channelId: '12345' });
    await execute(interaction);

    expect(interaction.reply).toHaveBeenCalledWith('Pausing queue');
    expect(redisClient.publish).toHaveBeenCalledWith(
      CHANNEL_EVENT_KEY,
      expect.stringContaining('"type":"pause"')
    );
...
it('loads module without DEBUG set (covers false branch of DEBUG ternary)', () => {
  ...
  jest.isolateModules(() => {
    ...
    require('../../../src/cmd_processor/commands/pause');
  });
