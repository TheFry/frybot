...
describe('skip command', () => {
  ...
  it('publishes skip event when user is in a voice channel', async () => {
    const interaction = createMockInteraction({ channelId: '12345' });
    await execute(interaction);

    expect(interaction.reply).toHaveBeenCalledWith('Skipping Song');
    expect(redisClient.publish).toHaveBeenCalledWith(
      CHANNEL_EVENT_KEY,
      expect.stringContaining('"type":"skip"')
    );
    const published = JSON.parse(redisClient.publish.mock.calls[0][1]);
    expect(published.channelId).toBe('12345');
    expect(published.type).toBe('skip');
  });
